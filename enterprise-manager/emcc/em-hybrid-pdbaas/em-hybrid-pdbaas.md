...
| **Step No.** | **Feature**                                                                | **Approx. Time** | **Description**                                                                                                                                                                      | **Summary**                                                                                                                                                                                                                   |
|--------|----------------------------------------------------------------------------|------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1    | Overview of SSA User and associated role                                      | 10min                     | Overview of SSA User and SSA Admin User.                                                                                  | Enterprise Manager contains 4 main out-of-the-box roles for the Cloud Self Service Portal, EM_CLOUD_ADMINISTRATOR - Users with this role can set up and manage the cloud infrastructure. This role is responsible for deploying the cloud infrastructure. EM_SSA_ADMINISTRATOR, EM_SSA_USER, EM_SSA_ADMINISTRATOR. In this lab we are using sysman user inplace of EM_CLOUD_ADMINISTRATOR. A user called 'cyrus' has been precreated and granted EM_SSA_USER role.You can check the documentation for more details - Defining roles and assigning users.                                                                                                           |
| 2    | Creation of PDBaaS Infra | 10min                     | Creation of PDBaaS Infrastucture     | The PDBaaS Infrastructure consists of PDBaaS Zone, PDB Pool,  Service Templates , Data Source, Quotas and Request settings. In this task we will explore most of these components to get a hands-on experience in building it.                                                |
| 3    | Self- service to request a PDB using PDBaaS         | 5min                      | Request PDB (pluggable database) using Service Catalogue on Private Cloud. Resize the PDB and then Delete the PDB while preserving the contents. | As a self service user we will explore the available options in PDBaaS. This includes provisioning of a PDB using Service template and restore a PDB using data source. A self service user could be a developer, or a BA or a application owner. They will be able to provision PDBs easily with very minuimal inputs. |
| 4    | Restore the PDB using Service Template                                | 5min                      | Restore the PDB using profile created in Task 3                                                                              | In this Task, we will be explore the option of a Self Service user to restore a PDB which has been provisioned in the earlier step. As a Self Service user, you will be able to choose the list of data profiles using which we will restore the PDB.    |
| 5    | Self Service to patch a PDB | 10min    |  Patch a pluggable database as a SSA User | Patching is generally an time and resource intensive actviity. Fleet Maintenance integrates its functionality with Cloud Management Pack to provide the ability of patching a pluggable database to SSA users without compromising the security and standardisations of the organistion |
